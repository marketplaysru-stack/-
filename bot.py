...
def vk_upload_photo(vk_session, image_url, owner_id=None):
    """
    Загружает фото в VK.
    Если owner_id < 0 – загружает на стену группы (используется group_id=abs(owner_id)).
    Если owner_id > 0 – загружает на личную стену.
    ...
    try:
        ...
        if owner_id is not None and owner_id < 0:
            # Группа – используем photo_wall с group_id
            photo = upload.photo_wall(photos=[image_data], group_id=str(abs(owner_id)))
            # В ответе photo[0] содержит owner_id (отрицательный) и id
            ...
            # fallback – если owner_id не указан (не должно случаться)
...
def post_to_vk_group(vk_session, group_id, text, image_url):
    ...
    try:
        # group_id может быть строкой с минусом или без; приводим к int
        group_id_int = int(group_id)
        owner_id = -abs(group_id_int)  # для группы owner_id отрицательный
...
def post_to_user_wall(vk_session, user_id, text, image_url):
    ...
    try:
        owner_id = int(user_id)  # положительное число
        attachment = vk_upload_photo(vk_session, image_url, owner_id=owner_id)
        if not attachment:
            attachments = []
        else:
