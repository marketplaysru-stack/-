...
Использует проверенный метод загрузки фото через временный файл (как в AI Навигаторе).
"""
...
def upload_photo_to_vk_via_vkapi(image_bytes, owner_id, token):
    """
    Загружает фото на стену через VkUpload с использованием временного файла.
