...
# -*- coding: utf-8 -*-
"""
Бот для родительского навигатора (загрузка фото через vk_api)
"""
...
# ========== ЗАГРУЗКА ФОТО В ГРУППУ (ЧЕРЕЗ vk_api) ==========
def upload_photo_to_group_via_vkapi(image_bytes, group_id, token):
    """Загружает фото в группу через vk_api (надёжно)"""
    try:
        ...
        photo = upload.photo_wall(temp_path, group_id=group_id_abs)
        os.remove(temp_path)
        if photo and isinstance(photo, list) and len(photo) > 0:
            photo_data = photo[0]
            attachment = f"photo{photo_data['owner_id']}_{photo_data['id']}"
            logger.info(f"Получен attachment через vk_api: {attachment}")
            return attachment
        else:
            raise Exception("Не удалось загрузить фото (пустой ответ)")
    except Exception as e:
        logger.error(f"Ошибка загрузки фото через vk_api: {e}")
        raise

# ========== ЗАГРУЗКА ФОТО НА ЛИЧНУЮ СТРАНИЦУ (ПРЯМОЙ API) ==========
def upload_photo_to_user_direct(image_bytes, user_id, token):
    """Загружает фото на личную стену через прямой API (работает для пользователей)"""
    try:
        ...
        attachment = f"photo{photo_data['owner_id']}_{photo_data['id']}"
        logger.info(f"Получен attachment для личной стены: {attachment}")
        return attachment
    except Exception as e:
...
def publish_to_group(text, image_bytes):
    ...
    try:
        ...
        if image_bytes:
            try:
                attachment = upload_photo_to_group_via_vkapi(image_bytes, GROUP_ID_AI, VK_TOKEN_AI)
                attachments.append(attachment)
                logger.info("Фото загружено в группу через vk_api")
            except Exception as e:
...
def publish_announce_to_user(announce_text, image_bytes, group_link):
    ...
    try:
        ...
        if image_bytes:
            try:
                attachment = upload_photo_to_user_direct(image_bytes, VK_USER_ID, VK_TOKEN_USER)
                attachments.append(attachment)
