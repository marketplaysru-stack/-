...
# -*- coding: utf-8 -*-
"""
Бот для родительского навигатора (прямая загрузка фото через VK API с двумя попытками)
"""
...
# ========== ПРЯМАЯ ЗАГРУЗКА ФОТО С ДВУМЯ ПОПЫТКАМИ ==========
def upload_photo_to_vk_direct(image_bytes, owner_id, token):
    """Загружает фото через прямой API с двумя вариантами сохранения"""
    try:
        ...
        try:
            ...
            logger.info(f"Ответ сохранения (с group_id): {json.dumps(save_resp)}")
            if "error" in save_resp:
                raise Exception(f"Ошибка сохранения: {save_resp['error']['error_msg']}")
            photo_data = save_resp["response"][0]
            attachment = f"photo{photo_data['owner_id']}_{photo_data['id']}"
            logger.info(f"Получен attachment: {attachment}")
            return attachment
        except Exception as e1:
            logger.warning(f"Первая попытка сохранения не удалась: {e1}")
            # Попытка 2: без group_id (только для публичных страниц)
            if owner_id > 0:
                try:
                    logger.info("Пробуем сохранить без group_id")
                    params_no_group = {
                        "access_token": token,
                        "photo": resp_upload["photo"],
                        "server": resp_upload["server"],
                    ...
                    save_resp2 = requests.post(save_api, data=params_no_group).json()
                    logger.info(f"Ответ сохранения (без group_id): {json.dumps(save_resp2)}")
                    if "error" in save_resp2:
                        raise Exception(f"Ошибка сохранения без group_id: {save_resp2['error']['error_msg']}")
                    photo_data = save_resp2["response"][0]
                    attachment = f"photo{photo_data['owner_id']}_{photo_data['id']}"
                    logger.info(f"Получен attachment (без group_id): {attachment}")
                    return attachment
        ...
        logger.error(f"Ошибка загрузки фото через прямой API: {e}")
        raise
...
def publish_to_group(text, image_bytes):
    ...
    try:
        ...
        if image_bytes:
            try:
                attachment = upload_photo_to_vk_direct(image_bytes, GROUP_ID_AI, VK_TOKEN_AI)
...
def publish_announce_to_user(announce_text, image_bytes, group_link):
    ...
    try:
        ...
        if image_bytes:
            try:
                attachment = upload_photo_to_vk_direct(image_bytes, VK_USER_ID, VK_TOKEN_USER)
                attachments.append(attachment)
