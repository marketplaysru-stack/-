...
Использует проверенный метод загрузки фото через vk_api.upload.
"""
...
# ========== VK ПУБЛИКАЦИЯ (НАДЁЖНЫЙ МЕТОД ЧЕРЕЗ VkUpload) ==========
def upload_photo_to_vk_via_vkapi(image_bytes, owner_id, token):
    """
    Загружает фото на стену через VkUpload.
    Для групп (owner_id < 0) – с group_id.
    Для публичных страниц и личных стен (owner_id > 0) – без group_id.
    ...
    try:
        ...
        if owner_id < 0:
            ...
            photo = upload.photo_wall([image_bytes], group_id=group_id)
            logger.info(f"Фото загружено в группу {group_id}")
        else:
            photo = upload.photo_wall([image_bytes])
