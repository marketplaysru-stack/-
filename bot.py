...
# -*- coding: utf-8 -*-
"""
Бот для родительского навигатора (прямая загрузка фото через VK API)
"""
...
# ========== ПРЯМАЯ ЗАГРУЗКА ФОТО В VK (БЕЗ vk_api) ==========
def upload_photo_to_vk_direct(image_bytes, owner_id, token):
    """Загружает фото через прямой API (работает для групп и публичных страниц)"""
    try:
        ...
        logger.info(f"Ответ сохранения: {json.dumps(save_resp)}")
...
def publish_announce_to_user(announce_text, image_bytes, group_link):
    ...
    try:
        ...
        if image_bytes:
            try:
                # Для личной страницы используем тот же прямой метод, но без group_id (или с owner_id)
                # Можно использовать тот же upload_photo_to_vk_direct, передавая owner_id = VK_USER_ID (положительный)
                # Он отработает как публичная страница, но с group_id = VK_USER_ID
