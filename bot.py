...
Картинки генерируются через нейросети с улучшенными промптами и повторными попытками.
Авто-постинг каждые 6 часов. Кэш картинок с автопилотом.
"""
...
# ---------- КЭШ КАРТИНОК (С АВТООЧИСТКОЙ) ----------
def load_used_images():
    if os.path.exists(USED_IMAGES_FILE):
        with open(USED_IMAGES_FILE, "r") as f:
            data = json.load(f)
            # Если структура старая (просто список) – конвертируем
...
def save_used_images(used_set):
    # Сохраняем с датой для возможной очистки
    data = {
        "hashes": list(used_set),
...
def clean_used_images():
    """Удаляет записи старше 7 дней (чтобы кэш не разрастался)"""
...
def generate_template_text(topic: str) -> str:
    ...
    leads = [
        f"Ребёнок жалуется на живот, хотя температуры нет? Стал раздражительным? Спойлер: он не симулянт. Он в стрессе. {topic} – это реальная проблема, которую мы часто игнорируем.",
        f"Ты когда-нибудь замечал, что твой ребёнок перестал делиться новостями из школы? Возможно, ты столкнулся с {topic}. Давай разберёмся.",
        f"Воспитывать детей – как вести корабль по бурному морю. {topic} – одна из тех тем, о которых страшно говорить, но нужно."
    ]
    ...
    body_pool = [
        ...
        ("❤️", "Любовь без условий", "Даже когда ребёнок ошибается, он должен знать, что его любят."),
        ("🤝", "Совместное решение", "Обсуждайте проблемы вместе – дети чувствуют себя важными."),
        ("🌟", "Похвала за усилия", "Хвалите не за результат, а за старания – это формирует самооценку."),
        ("📌", "Личный пример", "Твоё поведение – главный урок."),
        ("💡", "Игровой подход", "Учите через игру – дети лучше усваивают информацию."),
        ("🛡️", "Безопасная среда", "Ребёнок должен знать, что дом – это место, где его защитят."),
        ("🎯", "Цели и мечты", "Помогайте ставить цели, но не навязывайте свои.")
    ]
    random.shuffle(body_pool)
    selected = body_pool[:random.randint(3, 5)]
    body = "\n".join([f"{emoji} **{title}**\n{desc}" for emoji, title, desc in selected])
    conclusions = [
        f"Помните: {topic} – это возможность стать ближе к ребёнку.",
        "Каждый ребёнок уникален. Не бойтесь ошибаться.",
        "Лучшее, что вы можете дать ребёнку – это время и внимание."
    ]
    conclusion = random.choice(conclusions)
    cta_questions = [
        f"👇 А вы сталкивались с ситуациями, когда {topic} был для вас вызовом?",
        f"👇 Что помогает вам сохранять спокойствие в общении с детьми?",
        f"👇 Согласны, что иногда самое сложное – просто быть рядом?"
    ]
    cta = random.choice(cta_questions)
    comments_themes = [
        "1. «Как вы справляетесь с капризами ребёнка?»",
        "2. «Какую книгу по воспитанию вы считаете самой полезной?»",
        "3. «Что бы вы хотели изменить в своём подходе к воспитанию?»"
    ]
...
# ---------- УЛУЧШЕННЫЕ ГЕНЕРАТОРЫ КАРТИНОК ----------
def download_image_with_retry(url, timeout=30, retries=3):
    for attempt in range(retries):
        try:
            ...
            logger.warning(f"Попытка {attempt+1} скачать {url} не удалась: {e}")
            time.sleep(2)
    return None
...
def generate_agnes_image(prompt):
    ...
    try:
        ...
        full_prompt = f"Professional illustration about {prompt}, parenting, family, children, warm colors, high quality, detailed, sharp, no people, no nature"
        headers = {"Authorization": f"Bearer {AGNES_API_KEY}", "Content-Type": "application/json"}
        data = {
            "prompt": full_prompt,
...
def generate_pollinations_image(prompt):
    try:
        seed = random.randint(1, 1000000)
        full_prompt = f"{prompt}, parenting, family, children, warm colors, high quality, detailed, sharp, no people, no nature"
...
def search_pexels_relevant_photo(topic):
    if not PEXELS_API_KEY:
        return None
    queries = [f"parenting {topic}", f"family {topic}", f"children {topic}"]
...
def sharpen_image(image_bytes):
    try:
        ...
        img = enhancer.enhance(1.8)  # усиление резкости
        buf = io.BytesIO()
...
def generate_image(topic):
    # Автоочистка кэша, если он слишком старый
    clean_used_images()
    ...
    for name, func in generators:
        ...
        if img:
            ...
            if not is_image_used(img):
                ...
                logger.info(f"✅ Картинка сгенерирована через {name} (уникальная, резкая)")
                return img, name
            else:
                logger.info(f"⚠️ Картинка от {name} уже использовалась, пробуем следующий")

    # Если генераторы не дали уникальных – пробуем Pexels
...
def upload_photo_to_vk_via_http(image_bytes, owner_id, token):
    try:
        ...
        if 'photo' not in upload_data or 'server' not in upload_data or 'hash' not in upload_data:
            logger.error(f"Неполный ответ сервера: {upload_data}")
            return None
        save_params = {
...
def create_post_content(title, text=None):
    if text and len(text) > 50:
        ...
        if AGNES_API_KEY:
            try:
                headers = {"Authorization": f"Bearer {AGNES_API_KEY}", "Content-Type": "application/json"}
                prompt = f"Перепиши следующий текст, чтобы он стал более żywy, добавь emoji, akapity, сделай jak пост popularnego blogera o воспитании. Текст:\n\n{text}"
                data = {
                    "model": "agnes-v1",
                    "messages": [{"role": "user", "content": prompt}],
                    "max_tokens": 600,
...
# ---------- ЗАПУСК ----------
def main():
    logger.info("🚀 Бот запущен (улучшенная генерация картинок)")
