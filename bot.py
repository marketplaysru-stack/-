...
# -*- coding: utf-8 -*-
"""
Родительский навигатор – генерация позитивных и игровых постов.
...
# ---------- ГЕНЕРАЦИЯ ИГРОВЫХ ФОРМАТОВ ----------
def generate_quiz(topic):
    """Генерирует викторину по теме"""
    # Набор вопросов для разных тем (можно расширить)
    questions = [
        {
            "q": "Какой возраст считается оптимальным для начала обучения чтению?",
            "options": ["3-4 года", "5-6 лет", "7-8 лет", "9-10 лет"],
            "answer": "5-6 лет"
        },
        {
            "q": "Что помогает детям лучше усваивать информацию?",
            "options": ["Повторение", "Игровая форма", "Дисциплина", "Все варианты"],
            "answer": "Все варианты"
        },
        {
            "q": "Сколько времени рекомендуется проводить ребёнку за экраном в день?",
            "options": ["1 час", "2 часа", "3 часа", "Не ограничивать"],
            "answer": "1-2 часа"
        },
        {
            "q": "Что важнее всего для развития эмоционального интеллекта?",
            "options": ["Обсуждение чувств", "Чтение книг", "Спорт", "Музыка"],
            "answer": "Обсуждение чувств"
        },
        {
            "q": "Как помочь ребёнку справиться со страхом темноты?",
            "options": ["Ночник", "Сказки", "Обсуждение", "Все варианты"],
            "answer": "Все варианты"
        }
    ]
    # Если тема содержит ключевые слова, подбираем вопрос, иначе случайный
...
def generate_challenge(topic):
    return f"🏆 **Челлендж недели: {topic}**\n\nВаше задание: каждый день в течение недели уделять ребёнку 15 минут без отвлечений (без телефона).\n\nДелитесь результатами в комментариях! 💬"

def generate_riddle(topic):
    riddles = [
...
def generate_standard_post(topic: str) -> str:
    ...
    conclusions = [
        ...
        "Пусть каждый ваш день будет наполнен улыбками и гордостью за ваших dzieci."
    ]
...
def generate_text(topic: str) -> str:
    # Случайно выбираем игровой формат с вероятностью 50%
    if random.random() < 0.5:
        ...
        if game_type == "quiz":
            ...
            return generate_challenge(topic)
        elif game_type == "riddle":
            return generate_riddle(topic)
    else:
        # Обычный позитивный пост
...
def create_post_content(title, text=None):
    if text and len(text) > 50:
        ...
        if AGNES_API_KEY:
            try:
                headers = {"Authorization": f"Bearer {AGNES_API_KEY}", "Content-Type": "application/json"}
                prompt = f"Перепиши следующий текст, чтобы он стал более живым, добавь эмодзи, абзацы, сделай его как пост популярного блогера о воспитании z akcentem na pozytyw. Текст:\n\n{text}"
                data = {
                    "model": "agnes-v1",
...
# ---------- ЗАПУСК ----------
def main():
    logger.info("🚀 Бот запущен (позитивные посты + игровые форматы)")
