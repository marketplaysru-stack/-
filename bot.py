...
# -*- coding: utf-8 -*-
"""
Бот для родительского навигатора (с vk_api для загрузки фото)
"""
...
# ========== VK ПУБЛИКАЦИЯ (С ИСПОЛЬЗОВАНИЕМ vk_api) ==========
def upload_photo_to_vk_via_vkapi(image_bytes, owner_id, token):
    """Загружает фото в группу или на стену пользователя через vk_api"""
    try:
        ...
        # Определяем, для группы или пользователя загружаем
        if owner_id < 0:  # группа
            group_id = abs(owner_id)
            photo = upload.photo_wall(temp_path, group_id=group_id)
        else:  # пользователь или публичная страница (owner_id > 0)
            # Для публичных страниц (положительный ID) используем group_id = owner_id
            # но upload.photo_wall для публичных страниц не требует group_id, он определяет по токену
            # однако для публичных страниц лучше явно указать group_id
            if owner_id > 0:
                # Публичная страница
                photo = upload.photo_wall(temp_path, group_id=owner_id)
            else:
                # Пользователь
