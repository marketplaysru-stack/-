...
# -*- coding: utf-8 -*-
"""
Родительский навигатор – исправленная загрузка фото для публичных страниц.
"""
...
# ========== VK ПУБЛИКАЦИЯ (ИСПРАВЛЕННАЯ) ==========
def upload_photo_to_vk_via_http(image_bytes, owner_id, token):
    """
    Загружает фото на стену (группы или публичной страницы).
    Для owner_id < 0 (группа) – передаём group_id.
    Для owner_id > 0 (публичная страница, личная стена) – НЕ передаём group_id.
    """
    try:
        ...
        if owner_id < 0:
            ...
            # Публичная страница или личная стена – без group_id
        ...
        # Для owner_id > 0 – group_id не передаём
