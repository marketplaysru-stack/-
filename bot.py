...
logging.basicConfig(
    level=logging.DEBUG,  # Теперь видны все отладочные сообщения
...
GROUP_ID_AI = os.getenv('GROUP_ID_AI')
...
def extract_keywords(text, max_words=5):
    """Выделяет ключевые слова из текста для поиска в Pexels"""
    ...
    # Если ключевых слов нет, берём первые слова
    if not keywords:
...
def generate_text_agnes(topic):
    """Генерация текста поста через Agnes API"""
...
def get_agnes_image(prompt):
    """Генерация картинки через Agnes (если поддерживается)"""
    logger.debug(f"Попытка генерации картинки через Agnes с промптом: {prompt[:50]}...")
    try:
        # У Agnes нет встроенного генератора изображений, поэтому возвращаем None
        # Если в будущем появится, добавьте код здесь
        logger.debug("Agnes не поддерживает генерацию картинок, пропускаем")
        return None
    except Exception as e:
        logger.error(f"Ошибка Agnes при генерации картинки: {e}")
        return None

def get_pexels_image(query):
    """Поиск фото в Pexels по ключевым словам"""
    ...
    try:
        ...
        if photos:
            # Берём первое фото, но можно рандомизировать
...
def get_pixazo_image(prompt):
    """Генерация картинки через Pixazo API"""
...
def generate_banner(text):
    """Генерация простого баннера с текстом (резерв)"""
    logger.debug("Генерация баннера с текстом")
    try:
        img = Image.new('RGB', (1024, 768), color=(73, 109, 137))
        draw = ImageDraw.Draw(img)
        # Попробуем загрузить шрифт, если нет - используем дефолтный
        try:
        ...
        buf.seek(0)
        # Загружаем на imgbb (если есть ключ) или возвращаем байты
        # В данном случае возвращаем URL, загруженный через imgbb
        ...
        # Если нет ключа, сохраняем локально и возвращаем путь (но для VK нужен URL)
        # Вернём None, чтобы бот использовал другой метод
...
def generate_image_for_post(topic):
    """Цепочка генераторов в порядке приоритета (от лучшего к худшему)"""
    logger.info(f"Начинаем генерацию картинки для темы: {topic}")

    # 1. Agnes (если бы поддерживала картинки)
    img = get_agnes_image(topic)
    if img:
        logger.info("✅ Картинка от Agnes")
        return img, 'Agnes'

    # 2. Pexels (реальные фото)
    ...
    # 3. Pixazo (генеративное ИИ среднего качества)
    img = get_pixazo_image(topic)
    if img:
        logger.info("✅ Картинка от Pixazo")
        return img, 'Pixazo'

    # 4. Pollinations (улучшенный)
...
def vk_upload_photo(vk_session, image_url, group_id=None):
    """Загружает фото в VK (группу или личную стену) и возвращает attachment"""
    logger.debug(f"Загрузка фото из {image_url} в VK, группа: {group_id}")
    try:
        ...
        # Используем vk_api для загрузки
        upload = VkUpload(vk_session)
        photo = upload.photo_messages(photos=[image_data], peer_id=group_id) if group_id else upload.photo_wall(photos=[image_data])
        # Формируем attachment
        if group_id:
            # Для группы
            owner_id = int(group_id)
            attachment = f"photo{owner_id}_{photo[0]['id']}"
        else:
            # Для личной стены
            owner_id = vk_session.token['user_id']  # или из VK_USER_ID
            attachment = f"photo{owner_id}_{photo[0]['id']}"
...
def post_to_vk_group(vk_session, group_id, text, image_url):
    """Публикация поста в группу VK"""
    logger.info(f"Публикация в группу {group_id}")
    try:
        # Загружаем фото
        attachment = vk_upload_photo(vk_session, image_url, group_id)
        if not attachment:
            logger.error("Не удалось загрузить фото, публикуем без фото")
            attachments = []
        else:
...
def post_to_user_wall(vk_session, user_id, text, image_url):
    """Публикация на личную стену пользователя"""
    logger.info(f"Публикация на личную стену пользователя {user_id}")
    try:
        attachment = vk_upload_photo(vk_session, image_url, group_id=None)
        if not attachment:
            attachments = []
        else:
            attachments = [attachment]
...
async def post_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if not image_url:
            await update.message.reply_text("❌ Не удалось создать картинку, пост не опубликован.")
            return
        ...
        if result:
            ...
            # Также публикуем на личную стену, если есть токен пользователя
...
def rss_post_job():
    """Задача для автоматического постинга из RSS"""
    logger.info("Запуск RSS-постинга по расписанию")
    # Здесь должна быть логика чтения RSS и публикации
    # Для краткости оставляем заглушку – её можно доработать отдельно
    logger.debug("RSS-постинг выполнен (заглушка)")

# ======================== ЗАПУСК БОТА ========================

def main():
    # Запускаем RSS-планировщик в отдельном потоке
