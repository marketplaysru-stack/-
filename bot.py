...
# -*- coding: utf-8 -*-
"""
Родительский навигатор – генерация вдохновляющих, позитивных постов о детях, их росте и заботе.
Картинки генерируются через нейросети (Agnes → Pixazo → Pollinations) с яркими, тёплыми цветами.
...
# ---------- ЗАПАСНЫЕ ТЕМЫ (позитивный уклон) ----------
def load_topics():
    try:
...
# ---------- ГЕНЕРАЦИЯ ТЕКСТА (ПОЗИТИВНЫЙ, ВДОХНОВЛЯЮЩИЙ) ----------
def generate_text(topic: str) -> str:
    if AGNES_API_KEY:
        try:
            headers = {"Authorization": f"Bearer {AGNES_API_KEY}", "Content-Type": "application/json"}
            prompt = f"""
Напиши вдохновляющий, позитивный пост (около 250–300 слов) на тему: {topic} (воспитание, дети, родительство).
Пост должен нести радость, гордость, теплоту. Рассказывай о достижениях детей, их росте, умении преодолевать себя.
Удели внимание заботе и любви родителей, их вере в ребёнка.
Используй яркие, тёплые, жизнеутверждающие формулировки. Добавь эмодзи, разбивай на абзацы.
В конце — вопрос к читателям (CTA) и 3 темы для обсуждения.
"""
            data = {
                "model": "agnes-v1",
...
def generate_template_text(topic: str) -> str:
    ...
    # Вступление – тёплое, обнадёживающее
    leads = [
    ...
    # Тело – позитивные пункты с примерами
    body_pool = [
...
# ---------- ГЕНЕРАТОРЫ КАРТИНОК (тёплые, яркие тона) ----------
...
def create_post_content(title, text=None):
    if text and len(text) > 50:
        ...
        if AGNES_API_KEY:
            try:
                headers = {"Authorization": f"Bearer {AGNES_API_KEY}", "Content-Type": "application/json"}
                prompt = f"Перепиши следующий текст, чтобы он стал bardziej żywy, добавь emoji, akapity, сделай его jak пост popularnego blogera о воспитании z akcentem na pozytyw. Текст:\n\n{text}"
                data = {
                    "model": "agnes-v1",
                    "messages": [{"role": "user", "content": prompt}],
                    "max_tokens": 600,
...
def handle_command(chat_id, text):
    if text in ("/start", "/help"):
        send_message(chat_id,
            "👨‍👩‍👧 Бот «Родительский навигатор» – вдохновляющие посты!\n"
            "📌 Команды:\n"
...
# ---------- ЗАПУСК ----------
def main():
    logger.info("🚀 Бот запущен (позитивные посты, яркие картинки)")
