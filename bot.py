...
def generate_text(topic: str) -> str:
    if AGNES_API_KEY:
        try:
            ...
            prompt = f"Напиши развернутый пост (около 200 слов) на тему: {topic} (воспитание, дети, родительство). Используй факты, примеры, выводы. Добавь эмодзи и абзацы."
            data = {
                "model": "agnes-v1",
...
def generate_template_text(topic: str) -> str:
    intro_phrases = [
        f"👨‍👩‍👧 **{topic}** — важная тема для каждой семьи.\n",
        f"❤️ **{topic}** — вопрос, который волнует каждого родителя.\n",
        f"📚 **{topic}** — одна из самых актуальных тем в воспитании.\n"
    ]
    body_phrases = [
        "🗣️ Психологи рекомендуют начинать с доверительных бесед.",
        "🛡️ Безопасная среда и внимание к чувствам ребёнка — основа.",
        "🎮 Игровые методики помогают детям усваивать информацию легче.",
        "🌟 Хвалите за старания, а не только за результат.",
        "👨‍🏫 Личный пример родителей — лучший урок.",
        "🤝 Общайтесь с учителями, участвуйте в школьной жизни."
    ]
    conclusion_phrases = [
        "💪 С вашей поддержкой ребёнок справится с любыми трудностями!",
        "🌈 Помните, что каждый ребёнок уникален.",
...
def generate_agnes_image(prompt):
    ...
    try:
        ...
        headers = {"Authorization": f"Bearer {AGNES_API_KEY}", "Content-Type": "application/json"}
        full_prompt = f"Professional illustration about {prompt}, parenting, family, children, creative, modern, high quality, no people, no nature"
        data = {
            "prompt": full_prompt,
...
def generate_pollinations_image(prompt):
    try:
        seed = random.randint(1, 1000000)
        full_prompt = f"{prompt}, parenting, family, children, creative, modern, high quality, no people, no nature"
...
def generate_image(topic):
    """
    Генерирует картинку по теме поста:
    сначала нейросети (Agnes → Pixazo → Pollinations),
    затем Pexels, затем баннер.
    """
    # 1. Генерация через нейросети
    for generator in [
