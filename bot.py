...
# ======================== РАБОТА С VK (ИСПРАВЛЕННАЯ) ========================

def vk_upload_photo(vk_session, image_url, owner_id):
    """
    Загружает фото на стену (группы или пользователя) и возвращает attachment.
    owner_id должно быть отрицательным для группы, положительным для пользователя.
    """
    ...
    try:
        ...
        if owner_id < 0:
            # Группа – используем photo_wall с параметром group_id
            group_id = str(abs(owner_id))
            logger.debug(f"Загрузка в группу {group_id}")
            photo = upload.photo_wall(photos=[image_data], group_id=group_id)
            logger.debug(f"Ответ photo_wall для группы: {photo}")
        else:
            # Личная стена
            logger.debug(f"Загрузка на личную стену {owner_id}")
            photo = upload.photo_wall(photos=[image_data])
            logger.debug(f"Ответ photo_wall для личной стены: {photo}")
        ...
        logger.error(f"Ошибка загрузки фото в VK: {e}", exc_info=True)
        return None

def post_to_vk_group(vk_session, group_id, text, image_url):
    logger.info(f"Публикация в группу {group_id}")
    try:
        # Приводим group_id к int и делаем отрицательным
        group_id_int = int(group_id)
        owner_id = -abs(group_id_int)
        attachment = vk_upload_photo(vk_session, image_url, owner_id)
        if not attachment:
