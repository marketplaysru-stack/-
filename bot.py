...
def vk_upload_photo(vk_session, image_url, owner_id=None):
    """
    Загружает фото в VK.
    Если owner_id указан – загружает на стену этого пользователя/группы.
    Для группы owner_id должен быть отрицательным.
    Для личной стены – положительным.
    Если owner_id не указан – пытается загрузить как фото для сообщения (не используется).
    """
    ...
    try:
        ...
        if owner_id is not None and owner_id < 0:
            # Группа
            photo = upload.photo_messages(photos=[image_data], peer_id=str(owner_id))
            attachment = f"photo{owner_id}_{photo[0]['id']}"
        elif owner_id is not None and owner_id > 0:
            # Личная стена
            photo = upload.photo_wall(photos=[image_data])
            # Для личной стены owner_id фото — это id пользователя, который загрузил
            # Обычно это id владельца токена, но мы будем использовать переданный owner_id
            attachment = f"photo{owner_id}_{photo[0]['id']}"
        else:
            # fallback – загружаем как фото для сообщения (может не сработать для стены)
            photo = upload.photo_messages(photos=[image_data])
...
def post_to_vk_group(vk_session, group_id, text, image_url):
    ...
    try:
        owner_id = -int(group_id) if str(group_id).lstrip('-').isdigit() else int(group_id)
        attachment = vk_upload_photo(vk_session, image_url, owner_id=owner_id)
        if not attachment:
