...
def vk_upload_photo(vk_session, image_url, group_id=None):
    ...
    try:
        ...
        if group_id:
            photo = upload.photo_messages(photos=[image_data], peer_id=group_id)
            owner_id = int(group_id)
            attachment = f"photo{owner_id}_{photo[0]['id']}"
        else:
            photo = upload.photo_wall(photos=[image_data])
            owner_id = vk_session.token['user_id']  # может не работать, лучше использовать VK_USER_ID
            attachment = f"photo{owner_id}_{photo[0]['id']}"
...
def main():
    ...
    application.add_handler(CommandHandler("ping", ping))          # <-- новый обработчик
